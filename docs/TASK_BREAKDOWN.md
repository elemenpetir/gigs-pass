...
- [ ] Fix deteksi granted SSE — artikan "posisi null + pegang lock" = granted, bukan null saja (prasyarat Leave Queue, future-proofing semantik stream)
...
- [ ] Optimasi SSE waiting room — cache kategori per stream (hapus query PG steady-state) + broadcast posisi per-tick untuk skala puluhan ribu penunggu
