...
- **Keputusan Teknis / Catatan:**
  ...
  - **Model beban queue (design review):** ruang checkout itu pasif — buyer yang menunggu bayar tidak memegang koneksi/timer server (biaya hanya 1 row `awaiting_payment` + key Redis); laju admisi ter-cap 10 buyer/detik oleh valve batch 50/5s apa pun ukuran stok (stok besar = durasi rush lebih panjang, bukan hentakan lebih keras). Hotspot resource sesungguhnya = halaman tunggu: SSE polling per-koneksi tiap 2s → tiap penunggu memicu 1 query PG (`findCategory`, redundan) + 1 panggilan Redis per tick. Roadmap skala besar: cache kategori sekali per stream → broadcast posisi per-tick (registry in-process, biaya O(1) backend tak bergantung jumlah penunggu) → pub/sub bila multi-instance. Verifikasi empiris di Fase 15. Ruangan checkout penuh ≠ habis: selama ada lock, slot didaur ulang via TTL/cleanup; true sell-out hanya saat stock 0 + nol lock + antrian tersisa.
...
- [ ] Deteksi & siarkan true sell-out — cek O(1) sekali per tick di dequeuer: `stock == 0 && ZCARD(lockexpiry) == 0 && antrian tidak kosong` (= semua tiket lunas, tak ada lock menggantung; identitas `stock + lock aktif + lunas = quota` terjaga karena cleanup/release selalu INCR bersamaan hilangnya lock, dan confirmSlot menukar pasangan lock↔lunas tanpa menyentuh stock) → siarkan event `soldout` ke ruang tunggu → layar terminal di WaitingRoomPage + tutup stream. Selama masih ada lock kondisi tak pernah true — ruangan penuh tetap berarti menunggu daur ulang slot via TTL/cleanup, bukan habis. Sekalian: hapus panel `soldout` mayat di CheckoutPage (pemicu 409 out-of-stock sudah tak terjangkau sejak Admission=Lock).
- [ ] Hook `MAX_PENDING_LOCKS` — cap concurrent pending payment, wajib hidup saat integrasi payment gateway nyata (Xendit)
- [ ] Rate limiting berlapis — nginx `limit_req` per-IP longgar sebagai benteng luar (config-only, masuk rencana Fase 14) + `express-rate-limit` memory store: ketat di login/register dengan kalibrasi berbasis KEGAGALAN (sidik jari penyerang = banyak percobaan gagal, bukan banyak request), moderat per-user di queue join, global santai DI ATAS puncak trafik sah. Kaut: limit jangan memakan SSE `/stream` (auto-reconnect bisa jadi reconnect storm) dan audiens konser sering share satu IP (NAT kampus/venue) — pakai keying per-user untuk endpoint ber-auth. Anti-abuser yang *selalu berhasil* (stuffing kredensial valid / bot registrasi): Turnstile atau honeypot di `/register`, cek password bocor HaveIBeenPwned saat daftar, deteksi velocity akun-unik-per-IP — cukup dicatat; dibangun hanya saat sudah ada user & pembayaran nyata
