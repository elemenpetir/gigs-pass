...
- **Task Selanjutnya:** tidak ada task wajib tersisa (Fase 0-16 complete, monitoring 429 ditutup, screenshots live). Sisa backlog opsional bila waktu ada (Xendit, WebSocket, Leave Queue, dsb.) + 1 unit guardrail tertunda: `stress-test.yml` jadikan `TARGET_URL` input manual (anti-tembak prod). Verifikasi live selesai: domain gigspass.xyz + HTTPS aktif, smoke demo hijau (Event Orders dengan ad-blocker ON, landing ganti akun buyer ke organizer).
...
- **Keputusan Teknis / Catatan:** pindah ke `docs/DECISIONS.md` (21 entri, format Tanggal/Konteks/Keputusan/Konsekuensi). Yang tersisa di sini hanya status kerja; aturan bisnis mengikat ada di PRD + DECISIONS.
...
- [x] Known-limitation Redis re-init/rekonsiliasi (selesai-dengan-fallback, Sep 2026) — counter `stock:` yang hilang dibangun ulang otomatis dari DB (`quota − sold` di `listCategoriesByEvent`, terbukti live saat switch Upstash → Redis Cloud); antrian/lock memang ephemeral by design (rejoin + eager-expire, DECISIONS #1/#6). Job rekonsiliasi khusus tidak diperlukan.
