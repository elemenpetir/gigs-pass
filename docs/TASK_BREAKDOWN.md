...
- **Status Fase lain:** Fase 0-12 complete, Fase 13 complete, Event Category & Admission=Lock enhancement complete, Rate limiting (app layer) enhancement complete (unit test: 253, integration: 20).
...
- [ ] Rate limiting nginx `limit_req` sebagai benteng luar — config-only di EC2 host reverse proxy (masuk Fase 14); zone auth ~10r/m + general ~600r/m, `limit_req_status 429`. Lapisan lanjutan anti-abuser yang *selalu berhasil* (stuffing kredensial valid / bot registrasi) tetap dicatat: Turnstile atau honeypot di `/register`, cek password bocor HaveIBeenPwned saat daftar, deteksi velocity akun-unik-per-IP — dibangun hanya saat sudah ada user & pembayaran nyata
